...
def BringToLife(obj,plt,algo,vCalcfct,costfct,heuristicfct):
    ...
    # Plan path and give algorithm name "DstarLite" or "A*" #TODO: Make the algorithm call generic (via a call algorithm interface)
    # its the first run so construct the graph
    t0 = time.time() # performance measurement
    obj.AlgorithmModel['graph'] = AbstractToGraph(obj) 
    if algo == 'A*':
        t1 = time.time() # performance measurement
        # construct new graph
        path,cost = astar(auv = obj.AUV, env=obj.Environment, cost=costwithcurrents, heuristic=heuristicwithcurrents, vis=None, alpha = 1)
        t2 = time.time() # performance measurement
        obj.AUV.path = path
    elif algo == 'DstarLite':
        t1 = time.time() # performance measurement
        obj.AUV.path,cost,obj.AlgorithmModel['graph'] = DStarLite(obj.AlgorithmModel['graph'],(obj.AUV.origin[0],obj.AUV.origin[1]),(obj.AUV.goal[0],obj.AUV.goal[1]),obj,vCalcfct,costfct,heuristicfct)
        t2 = time.time() # performance measurement
        print(algo,": ","MaxQueueLength: ",obj.AlgorithmModel['graph'].graph['U'].maxlength)
    else:
        raise Exception('Please specify an algorithm for path planning!')
    
    print(algo,"(total): ","Plan-# ", obj.AlgorithmModel['runs']," in ... ", t2-t0, " seconds.") # performance print
    print(algo,"(without model construction): ","Plan-# ", obj.AlgorithmModel['runs']," in ... ", t2-t1, " seconds.") # performance print
    ...
    auvIm = ImageTk.PhotoImage(img_zoomed.rotate(90)) # we put it into the water in this orientation
    canvas.create_image(*GeometryToTkinter([(xCO,yCO)]), image = auvIm, anchor = CENTER)
    #TODO: Store cost on edges as weight
    # pack the canvas
    canvas.pack()    
    # move and explore
    i = 0
    while not finished and collisionFree:
        ...
        new_knowledge = False
        obj.AlgorithmModel['graph'].graph['updates'] = {'risk': {},'current': {}}
        for coord in coords:
            if obj.Environment.RiskField[coord[0]][coord[1]] != 1.0:
                new_knowledge = True
                # update 
                obj.AlgorithmModel['graph'].graph['updates']['risk'][coord] = 1.0 
        ...
        if finished != True and PathBlocked == True: # then replan  
            ...
            obj.AlgorithmModel['runs'] += 1 
            if algo == 'A*':
                obj.AUV.origin = (xCO,yCO,-9.5)
                t0 = time.time() # performance measurement
                # A* needs to start from the scratch so reconstruct the graph
                obj.AlgorithmModel['graph'] = AbstractToGraph(obj) 
                # Interface with A*
                t1 = time.time() # performance measurement
                path,_ = astar(auv = obj.AUV, env=obj.Environment, cost=costwithcurrents, heuristic=heuristicwithcurrents, vis=obj, alpha = 1)
                t2 = time.time() # performance measurement
                obj.AUV.path = path
                #print(algo,": ","MaxQueueLength: ",Qmax) # TBD later
            
            elif algo == 'DstarLite':
                t0 = time.time() # performance measurement
                #TODO: here we need to update the model (to measure update performance) (make an update function ...)
                t1 = time.time() # performance measurement
                obj.AUV.path,_,obj.AlgorithmModel['graph'] = DStarLite(obj.AlgorithmModel['graph'],(xCO,yCO),(obj.AUV.goal[0],obj.AUV.goal[1]),obj,vCalcfct,costfct,heuristicfct)
                t2 = time.time()
                print(algo,": ","MaxQueueLength: ",obj.AlgorithmModel['graph'].graph['U'].maxlength)
